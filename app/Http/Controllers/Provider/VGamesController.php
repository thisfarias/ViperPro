    ...
    public function show(string $slug, $aposta = null)
    {
        if (auth()->check()) {
            ...
            if (!empty($game)) {
                ...
                if (
                    GameBalance::join('game_exclusives', 'game_exclusives.uuid', '=', 'game_balances.game_slug')
                        ->join('categories', 'categories.id', '=', 'game_exclusives.category_id')
                        ->where('game_balances.user_id', auth()->user()->id)
                        ->where('categories.slug', 'exclusives')
                        ->where('game_balances.created_at', '>=', now()->subSeconds(5))
                        ->count() == 0
                ) {
                    if ($aposta) {
                        $wallet = Wallet::where('user_id', auth()->user()->id)->first();
                        if ($aposta > $wallet->balance) {
                            if ($aposta > $wallet->balance_bonus) {
                                return back()->with('error', 'Saldo insuficiente!');
                            }
                            $novoSaldo = $wallet->balance_bonus - (float)$aposta;
                            Wallet::where('user_id', auth()->user()->id)->update(['balance_bonus' => $novoSaldo]);
                        } else {
                    ...
                    return back()->with('error', 'Aguarde 5 segundos para jogar novamente');
                }
