...
const diff = decodeURIComponent(params.get("velo"));
let aposta = +decodeURIComponent(params.get("aposta"));
let xmeta = +decodeURIComponent(params.get("xmeta"));
var meta = aposta * xmeta;
let scene, camera, renderer, cube, orbitControl, animationFrame;
let worlds = [], laneWidth = 0.5, glow;
let sonic, sonicAction = "run", sonicHasCrashed = false, currentLane = 0, maxHeight = 0.5, dRot = 45,
    strafeDuration = 0.5, actionClock = {startTime: undefined, duration: 667},
...
var btnSair = () => {
    return document.querySelector('button#sair');
}
...
const startGame = () => {
    swal({
        ...
        text: (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) ? 'Deslize acima para pular, Deslize abaixo para rolar, esquerda e direita para trocar as rotas' : 'Use as setas do teclado para se mover.\nAcima para Pular, Abaixo para rolar, esquerda e direita para trocar as rotas',
        button: {
            text: "Jogar",
...
const addSea = () => {
    ...
    let seaMaterial = new THREE.MeshBasicMaterial({
        color: 0x2eccfa,
        transparent: true,
        opacity: 0.5,
    ...
    obstacles.forEach(obstacle => {
        if (obstacle.position.z > sonic.position.z + gap) {    //gone beyond field of view
            ...
            if (crashed) {
                ...
                document.getElementById('messagetitle').innerHTML = '<span class="perca">Tente outra vez!</span>';
                document.getElementById('modalmessage').innerHTML = '<h3 class="text-center">Você é fera</h3> <br> Infelizmente você deixou de ganhar <span class="reais"> R$ ' + Number(score).toFixed(2) + '</span><br> Mas não fique triste, você foi melhor que 87% das pessoas.';
...
const addScore = () => {
    ...
    scoreLabel.style.height = '35px';
    scoreLabel.style.fontSize = '35px';
    scoreLabel.style.fontFamily = 'Poppins, sans-serif';
    scoreLabel.style.fontWeight = 'bold';
    scoreLabel.style.color = '#282828';
    scoreLabel.style.textAlign = 'left';
    scoreLabel.style.lineHeight = '32px';
    scoreLabel.innerHTML = `R$: ${Number(score).toFixed(2)}`;
                    ...
                    if (sonic.position.x > obstacle.position.x - laneWidth / 2 && sonic.position.x < obstacle.position.x + laneWidth / 2 && obstacle.position.z + coin.position.z >= sonic.position.z) {
                        ...
                        if (coin.visible) {
                            ...
                            if (score > meta) {
                                ...
                                btnSair().onclick = function (event) {
                                    ...
                                    xhr.onreadystatechange = function () {
                                        if (xhr.readyState === 4 && xhr.status === 200) {
                                            ...
                                            window.parent.location.href = baseurl; //+ "?msg=" + encodeURIComponent(msg);
                                        } else if (xhr.readyState === 4) {
                                            console.error('Erro ao salvar o jogo', xhr.status);
                                ...
                                //     $('#myModal').data('bs.modal')._config.keyboard = false;
                                //     $('#myModal').data('bs.modal')._config.backdrop = 'static';
                                //     document.getElementById('messagetitle').innerHTML = '<span class="ganho">Você Ganhou!</span>';
                                //     document.getElementById('modalmessage').innerHTML = '<h3 class="text-center">Parabéns!</h3> <br> Você poderia ter ganhado <span class="reais"> R$ ' + score + '</span><br> Você tem potencial :) <br> Aproveite sua habilidade e se cadastre agora';
                                //     document.getElementById('avante').addEventListener('click', function () {
